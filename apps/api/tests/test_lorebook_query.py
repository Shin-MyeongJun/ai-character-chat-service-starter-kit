...
def data():
    ...
    return SimpleNamespace(
        lorebooks=lorebooks, owner_id=owner_id, other_owner_id=other_owner_id
    )
...
async def test_lorebook_cursor_pages_include_private_rows_and_handle_timestamp_ties(
    ...
    cursor = None
    while True:
        page = await query.list_lorebooks(
            database, LorebookTypes.ListLorebooksCommand(cursor=cursor, limit=2)
        )
...
async def test_owner_filter_is_preserved_across_lorebook_cursor_pages(database, data):
    first = await query.list_lorebooks_by_owner_id(
        database,
        LorebookTypes.ListLorebooksByOwnerIdCommand(owner_id=data.owner_id, limit=2),
    )
    second = await query.list_lorebooks_by_owner_id(
        database,
        LorebookTypes.ListLorebooksByOwnerIdCommand(
...
async def test_lorebook_detail_is_owner_scoped(database, data):
    result = await query.get_lorebook_by_id(
        database, LorebookTypes.GetLorebookByIdCommand(lorebook_id=uid(10))
    )
    owned = await query.get_lorebook_by_id_and_owner_id(
        database,
        LorebookTypes.GetLorebookByIdAndOwnerIdCommand(
            lorebook_id=uid(10), owner_id=data.owner_id
        ),
    )
    foreign = await query.get_lorebook_by_id_and_owner_id(
        database,
        LorebookTypes.GetLorebookByIdAndOwnerIdCommand(
            lorebook_id=uid(10), owner_id=data.other_owner_id
        ),
    )
    ...
    assert (
        await query.get_lorebook_by_id(
            database, LorebookTypes.GetLorebookByIdCommand(lorebook_id=uid(999))
        )
        is None
    )
...
async def test_entry_page_forwards_parent_cursor_and_limit(monkeypatch):
    cursor = types.LorebookCursor(
        created_at=datetime(2026, 9, 2, tzinfo=UTC), id=uid(201)
    )
    ...
    page = await query.list_lorebook_entries(
        ...
        LorebookTypes.ListLorebookEntriesCommand(
            lorebook_id=uid(10), cursor=cursor, limit=7
        ),
    )
...
async def test_owned_entry_list_checks_parent_before_reading_children(monkeypatch):
    parent = make_lorebook(10, uid(800), datetime(2026, 9, 1, tzinfo=UTC))
    get_parent = AsyncMock(side_effect=[None, parent])
    list_entries = AsyncMock(
        return_value=types.LorebookEntryPage(items=[make_entry(101)], next_cursor=None)
    )
    ...
    denied = await query.list_lorebook_entries_by_owner_id(
        session,
        LorebookTypes.ListLorebookEntriesByOwnerIdCommand(
            lorebook_id=uid(10), owner_id=uid(801)
        ),
    )
    ...
    page = await query.list_lorebook_entries_by_owner_id(
        session,
        LorebookTypes.ListLorebookEntriesByOwnerIdCommand(
            lorebook_id=uid(10), owner_id=uid(800), limit=3
        ),
    )
    ...
    list_entries.assert_awaited_once_with(session, uid(10), cursor=None, limit=3)
...
async def test_entry_lookup_keeps_entry_id_scoped_to_requested_parent(monkeypatch):
    ...
    owned = await query.get_lorebook_entry(
        session,
        LorebookTypes.GetLorebookEntryCommand(lorebook_id=uid(10), entry_id=uid(101)),
    )
    cross_parent = await query.get_lorebook_entry(
        session,
        LorebookTypes.GetLorebookEntryCommand(lorebook_id=uid(20), entry_id=uid(101)),
    )
...
async def test_owned_entry_lookup_stops_on_parent_owner_mismatch(monkeypatch):
    ...
    denied = await query.get_lorebook_entry_by_owner_id(
        session,
        LorebookTypes.GetLorebookEntryByOwnerIdCommand(
            lorebook_id=uid(10), entry_id=uid(101), owner_id=uid(801)
        ),
    )
    assert denied is None
    ...
    owned = await query.get_lorebook_entry_by_owner_id(
        session,
        LorebookTypes.GetLorebookEntryByOwnerIdCommand(
            lorebook_id=uid(10), entry_id=uid(101), owner_id=uid(800)
        ),
    )
    assert isinstance(owned, types.LorebookEntryInfo)
...
async def test_always_activation_requests_enabled_rows_and_preserves_order(monkeypatch):
    ...
    monkeypatch.setattr(
        repository, "list_enabled_entries_by_activation_type", list_enabled
    )
    session = object()
    active = await query.get_always_entries(
        session,
        LorebookTypes.GetAlwaysEntriesCommand(lorebook_ids=[uid(10), uid(20), uid(10)]),
    )
    ...
    assert all(isinstance(item, types.ActiveLorebookEntryInfo) for item in active)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("regex", "\\bdragons?\\b", "Two DRAGONS wake", True),
        ("regex", "^dragon$", "dragon wakes", False),
    ],
)
async def test_keyword_activation_match_modes_are_case_insensitive(
    monkeypatch, match_mode, trigger, text, matches
):
    entry = make_entry(101, key_triggers=[trigger], match_mode=match_mode)
    ...
    active = await query.activate_keyword_entries(
        session,
        LorebookTypes.ActivateKeywordEntriesCommand(
            lorebook_ids=[uid(10), uid(10)], text=text
        ),
    )
...
async def test_keyword_activation_preserves_repository_order_of_matching_rows(
    ...
    first = make_entry(
        101, key_triggers=["dragon"], match_mode="contains", priority=100
    )
    skipped = make_entry(
        102, key_triggers=["castle"], match_mode="contains", priority=50
    )
    second = make_entry(103, key_triggers=["dragons?"], match_mode="regex", priority=1)
    ...
    active = await query.activate_keyword_entries(
        object(),
        LorebookTypes.ActivateKeywordEntriesCommand(
            lorebook_ids=[uid(10)], text="A dragon wakes"
        ),
    )
...
async def test_invalid_stored_keyword_regex_raises_domain_error(monkeypatch):
    ...
    with pytest.raises(ValueError, match="invalid regular expression"):
        await query.activate_keyword_entries(
            object(),
            LorebookTypes.ActivateKeywordEntriesCommand(
                lorebook_ids=[uid(10)], text="dragon"
            ),
        )
