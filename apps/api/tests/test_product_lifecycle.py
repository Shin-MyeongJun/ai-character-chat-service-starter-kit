...
from app.modules.chatting.conversation.service import prepare_runtime_context
...
from app.modules.content.product.service.views.notices import (
    get_pending_updates,
    get_version_availability,
)
from app.modules.content.product.types import ReleaseNoteCommand
from app.modules.governance.admin import types as AdminTypes
from app.modules.governance.admin.service.command import set_expiry
from app.modules.llm import types as LlmTypes
from app.modules.llm.service.replacement import announce_retirement, resolve_execution
        ...
        async with AsyncSession(db.bind, expire_on_commit=False) as session:
            ...
            return await publish_product(
                ...
                ProductTypes.PublishCommand(
                    product_id=p.id,
                    owner_id=owner.id,
                    value=ReleaseNoteCommand("Concurrent", "Concurrent"),
                ),
            )
    ...
    with pytest.raises(ValueError, match="expired"):
        await prepare_runtime_context(
            ...
            ConversationTypes.PrepareRuntimeContextCommand(
                conversation_id=cid, user_id=owner.id
            ),
        )
    async with use_case_transaction(db):
...
async def test_model_retirement_plans_before_deadline_and_switches_once(db):
    ...
    async with use_case_transaction(db):
        customer = await get_pending_updates(
            db,
            ProductTypes.PendingUpdatesCommand(conversation_id=cid, user_id=owner.id),
        )
        creator = await get_version_availability(
            db,
            ProductTypes.VersionAvailabilityCommand(
                product_id=p.id, snapshot_id=release.snapshot_id, owner_id=owner.id
            ),
        )
        ...
        with pytest.raises(LookupError):
            await get_version_availability(
                db,
                ProductTypes.VersionAvailabilityCommand(
                    product_id=p.id, snapshot_id=release.snapshot_id, owner_id=uuid4()
                ),
            )
        planned = await resolve_execution(
            db,
            LlmTypes.ResolveExecutionCommand(snapshot_id=release.snapshot_id, now=now),
        )
        assert (
    ...
    async with use_case_transaction(db):
        executed = await resolve_execution(
            ...
            LlmTypes.ResolveExecutionCommand(
                snapshot_id=release.snapshot_id, now=shutdown - timedelta(days=1)
            ),
        )
