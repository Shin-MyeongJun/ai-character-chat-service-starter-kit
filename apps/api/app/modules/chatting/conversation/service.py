    ...
    async with session.begin():
        ...
        characters = list(
            await session.scalars(
                select(ProductSnapshotCharacter).where(
                    ProductSnapshotCharacter.product_snapshot_id == snapshot.id
                )
            )
        )
        ...
        session.add(
            Message(
                ...
                character_id=source.character_id,
                product_character_id=primary.id,
                product_snapshot_id=snapshot.id,
                content=snapshot.snapshot_data["opening_message"],
...
async def runtime_context(session, *, conversation_id, user_id):
    ...
    chars = list(
        await session.scalars(
            ...
            .where(ProductSnapshotCharacter.product_snapshot_id == snapshot.id)
    ...
    books = list(
        await session.scalars(
            ...
            .where(ProductSnapshotLorebook.product_snapshot_id == snapshot.id)
    ...
    targets = list(
        await session.scalars(
            select(ProductSnapshotLorebookCharacter).where(
                ProductSnapshotLorebookCharacter.product_snapshot_id == snapshot.id
            )
        )
    )
    character_data = []
