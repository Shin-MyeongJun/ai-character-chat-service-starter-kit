...
async def set_expiry(session, *, snapshot_id, actor_id, expires_at, reason):
    ...
    async with session.begin():
        ...
        session.add(ProductSnapshotPolicyChange(product_snapshot_id=snapshot.id,actor_id=actor_id,old_expires_at=snapshot.expires_at,new_expires_at=expires_at,reason=reason))
...
async def ensure_available(session, snapshot_id, *, now=None):
    ...
    if snapshot.expires_at is not None and snapshot.expires_at <= (now or datetime.now(UTC)):
        raise ValueError('Version expired. History and memories are preserved; choose another version.')
    return snapshot
