...
async def replace_composition(
    ...
    async with session.begin():
        ...
        for entity, ids in (
            (Character, [c.character_id for c in value.characters]),
            (Lorebook, [b.lorebook_id for b in value.lorebooks]),
        ):
            found = list(
                await session.scalars(
                    select(entity.id)
                    .where(entity.id.in_(ids), entity.owner_id == owner_id)
                    .order_by(entity.id)
                    .with_for_update()
                )
            )
        ...
        existing_chars = {
            ...
            for c in await session.scalars(
                select(ProductCharacter).where(
                    ProductCharacter.product_id == product_id
                )
            )
        }
        existing_books = {
