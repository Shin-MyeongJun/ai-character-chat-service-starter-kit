    ...
    async with session.begin():
        ...
        if value.replacement_model_ids:
            found = list(
                await session.scalars(
                    select(Model.id).where(Model.id.in_(value.replacement_model_ids))
                )
            )
        ...
        entries = list(
            await session.scalars(
                ...
                .join(
                    ProductLorebook,
                    ProductLorebook.lorebook_id == LorebookEntry.lorebook_id,
                )
                .where(
                    ProductLorebook.product_id == product_id,
                    LorebookEntry.id.in_(value.start_entry_ids),
...
async def validate_publication(session, product):
    ...
    chars = list(
        await session.scalars(
            select(ProductCharacter).where(ProductCharacter.product_id == product.id)
        )
    )
    ...
    starts = list(
        await session.scalars(
            select(ProductStartSet).where(ProductStartSet.product_id == product.id)
        )
    )
    if not starts:
    ...
    valid = list(
        await session.scalars(
            select(LorebookEntry.id).where(
                LorebookEntry.id.in_([s.entry_id for s in starts]),
                LorebookEntry.entry_type == "start_set",
                LorebookEntry.is_enabled.is_(True),
            )
        )
