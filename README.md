...
| 회원가입·로그인 백엔드 | 자체 가입·이메일 인증·재설정·Google OIDC·쿠키 세션 구현. 화면과 가입 후 서비스 흐름은 별도 |
...
이메일 가입부터 쿠키 로그인·첫 비스트리밍 답변까지의 실제 PostgreSQL 통합 테스트와 재현 절차는 [첫 답변 통합 검증](docs/first-answer-integration.md)에 정리했습니다. 외부 LLM 호출은 mock이며 실제 공급자 검증과 구분합니다.
...
현재 `docker-compose.yml`은 **PostgreSQL과 Redis를 실행**하며 `auth-dev` 프로필로 Mailpit을 추가할 수 있습니다. API와 프런트엔드를 포함한 전체 서비스의 Compose 실행은 향후 목표입니다.
...
현재 ASGI 진입점은 `apps/api/app/main.py`의 `app`입니다. 기본 owner 인증 훅은 JWT 쿠키와 서버 세션 검증에 연결됩니다. 기존 `create_app(authenticate=...)` 주입 계약도 유지합니다. 관리자/모더레이터 HTTP 권한 훅은 별도 연결 대상입니다. 인증 키·SMTP·Google 설정, Alembic 0020, API와 CSRF 사용법은 [회원가입·로그인 실행 문서](docs/authentication.md)를 참고하세요.
